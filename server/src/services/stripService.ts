...
const buildTextTab = (text: string): Buffer => {
  const safeText = sanitizeText(text.trim());
  const textContent = safeText || ' ';

  const tabSvg = `
    <svg width="${PHOTO_WIDTH}" height="${TAB_HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      <rect x="1" y="1" width="${PHOTO_WIDTH - 2}" height="${TAB_HEIGHT - 2}" rx="18" ry="18" fill="#F7F7F5" stroke="#DBDBD8" stroke-width="2" />
      <text x="50%" y="54%" dominant-baseline="middle" text-anchor="middle" fill="#1B1D22" font-size="52" font-family="Arial, sans-serif" letter-spacing="1.2">
        ${textContent}
      </text>
    </svg>
  `;
...
export const generatePhotoStrip = async (payload: GenerateStripPayload): Promise<Buffer> => {
  const { photos, filters, text = '', layout } = payload;
